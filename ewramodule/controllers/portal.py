...
class CustomerPortalInhert(portal.CustomerPortal):
    def _prepare_helpdesk_tickets_domain(self):
        ...
        if usertype:
            ...
            domains=[('partner_id','=',userid)]        
        return domains
    def _prepare_home_portal_values(self, counters):
        #values = super()._prepare_home_portal_values(counters)
        values={}
        values['ticket_count']=0
        #alltickets=request.env['helpdesk.ticket'].sudo().search([])
        if 'ticket_count' in counters:
            values['ticket_count'] = (
                request.env['helpdesk.ticket'].sudo().search_count(self._prepare_helpdesk_tickets_domain())
